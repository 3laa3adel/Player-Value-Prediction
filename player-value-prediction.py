...
X_train, X_test, y_train, y_test = train_test_split(
    X, data['value'], test_size=0.20, shuffle=True, random_state=10)
...
scores = cross_val_score(poly_model1, X_train_poly_model_1,
                         y_train, scoring='neg_mean_squared_error', cv=6)
...
if((metrics.mean_squared_error(y_test, prediction))**0.5 < data['value'].max()):
    print("True MSE")
else:
    print("False MSE")
...
if((metrics.mean_squared_error(np.asarray(y_test), prediction))**0.5 < data['value'].max()):
    print("True MSE")
else:
    print("False MSE")
print("--- %s seconds ---" % (time.time() - start_time))
print('accuracy = ', M_L_R.score(X_test, y_test))
